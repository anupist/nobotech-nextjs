...
export function HomePage() {
  ...
  const [featuredFlashSale, setFeaturedFlashSale] = useState<FlashSale | null>(null)
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true)
      try {
        const [bannersRes, categoriesRes, featuredRes, newArrivalRes, bestSellerRes, brandsRes, testimonialsRes, flashSaleRes] =
          await Promise.all([
            fetchBanners('hero'),
            fetchCategories(),
        ...
        setFeaturedFlashSale(flashSaleRes.data || null)
      } catch {
  ...
  useEffect(() => {
    if (!featuredFlashSale?.endsAt) return

    const endTime = new Date(featuredFlashSale.endsAt)
  ...
  return (
    <div className="space-y-16 pb-12 hero-parallax-bg">
        ...
        >
          ...
          >
            {categories.slice(0, 8).map((cat) => {
              const CatIcon = cat.icon ? Store : Store
              return (
                <motion.button
                  key={cat.id}
                  ...
                  {...staggerChild}
                >
                  <div className="h-14 w-14 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center shadow-md group-hover:shadow-lg transition-shadow duration-300">
      ...
      {/* Flash Sale */}
      {featuredFlashSale && featuredFlashSale.products.length > 0 && (
        <motion.section
          ...
          <div className="container mx-auto px-4 py-10 relative">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">{featuredFlashSale.name}</h2>
              ...
              <div className="flex items-center gap-4">
                ...
                <Button
                  variant="outline"
                  className="border-white/40 text-white hover:bg-white/15 backdrop-blur-sm transition-all duration-300"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {featuredFlashSale.products.slice(0, 5).map((fp) => (
                <div
                  key={fp.id}
                  className="bg-white rounded-xl overflow-hidden shadow-xl ring-1 ring-white/50"
                >
                  <ProductCard product={{ ...fp.product, discountPrice: fp.salePrice }} />
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div className="bg-card rounded-xl p-6 shadow-sm h-full">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <img
                          src={t.avatar || 'https://i.pravatar.cc/100?u=default'}
                          alt={t.name}
